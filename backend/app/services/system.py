...
from app.schemas.system import AgentContext, ModelSettings, SystemStats
    ...
    try:
        if os.path.exists(MODEL_SETTINGS_FILE):
            with open(MODEL_SETTINGS_FILE, "r") as f:
                data = json.load(f)
                return {
                    name: ModelSettings(**settings) for name, settings in data.items()
                }
        return {}
...
async def update_model_settings(
    model_name: str, settings: ModelSettings
) -> ModelSettings:
